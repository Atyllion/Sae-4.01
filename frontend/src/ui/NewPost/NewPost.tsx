...
export default function NewPost() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (!content.trim() && mediaFiles.length === 0 && e.type === 'submit') {
        ...
        <>
            <div className="flex flex-col items-center justify-center bg-[var(--bg-color)] rounded-lg p-4 w-full md:w-full">
                ...
                <form onSubmit={handleSubmit} className="w-full">
                            ...
                            {mediaPreviews.map((preview, index) => (
                                <div key={index} className="relative">
                                    ...
                                    <DynamicButton
                                        ...
                                        variant="danger"
                                        className="absolute top-2 right-2 rounded-full w-6 h-6 p-0 min-w-0 flex items-center justify-center"
                                        icon={<span className="text-sm">&times;</span>}
                                        size="small"
                                    />
                                </div>
                            ))}
                    ...
                    <DynamicButton
                        label={isSubmitting ? "Publication en cours..." : "Publier"}
                        onClick={() => { }}
                        variant="primary"
